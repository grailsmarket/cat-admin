...
import { fetchRequestAnalytics } from '@/api/analytics'
...
export default function RequestAnalyticsPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['analytics-requests', dateRange.from, dateRange.to],
    queryFn: () => fetchRequestAnalytics(dateRange.from, dateRange.to),
  })

  const { data: drilldownData } = useQuery({
    queryKey: ['analytics-requests-user', dateRange.from, dateRange.to, selectedUser],
    queryFn: () => fetchRequestAnalytics(dateRange.from, dateRange.to, selectedUser!),
  ...
  // Resolve ENS names for top users
  useEffect(() => {
  ...
  // Merge daily data with user drilldown data for chart overlay
  ...
  return (
    <div className='p-4 lg:p-8'>
      ...
      <div className='mb-8'>
        ...
        <p className='text-neutral mt-1'>Track API request volume by route and user</p>
      </div>
      ...
      <div className='mb-6 grid grid-cols-3 gap-4'>
        <div className='card'>
          <p className='text-neutral text-sm font-medium'>Total Requests</p>
          {isLoading ? (
        ...
        <div className='card'>
          <p className='text-neutral text-sm font-medium'>Unique Users</p>
          {isLoading ? (
            <div className='mt-1 h-9 w-20 animate-pulse rounded bg-tertiary' />
          ) : (
            <p className='mt-1 text-3xl font-bold'>
        ...
        <div className='card'>
          <p className='text-neutral text-sm font-medium'>Unique Routes</p>
          {isLoading ? (
            <div className='mt-1 h-9 w-20 animate-pulse rounded bg-tertiary' />
          ) : (
            <p className='mt-1 text-3xl font-bold'>
              {(summary?.uniqueRoutes ?? 0).toLocaleString()}
            </p>
          )}
        </div>
      </div>
      ...
      <div className='card mb-6'>
        ...
        {isLoading ? (
          ...
          <div className='rounded-lg border border-error bg-error/10 p-4 text-sm text-error'>
            {data?.error || 'Failed to load request analytics'}
          </div>
